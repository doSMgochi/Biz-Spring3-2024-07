...
public class HomeController {
	...
	public HomeController(StudentDao studentDao, StudentService studentService) {
		super();
	...
	public String stNumCheck(String st_num) {
		/*
		 * findById(id) method 는 PK 를 기준으로 SELECT 하여 그 결과를 return 한다. 만약 id 에 해당하는 데이터가
		 * 있으면 그 데이터를 return 하고 없으면 null 값을 return 한다
		 */
...
//		return "home";
	...
	public String update(String num, Model model) {
		...
		model.addAttribute("ST", studentVO);
		// update 를 실행할 때 새로운 update.jsp 를 만들지 않고
		// input.jsp 를 재활용 하겠다
		return "student/input";
	}
