 ...
 * Custom Class
 * DB 로부터 사용자 정보를 조회하고, 하는 등등을 처리해야 한다. 
 */
...
public class AuthProviderImpl implements AuthenticationProvider{
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		// login.jsp 에서 form 에 담겨 전달된 username(u_name)과 password(u_pass)의 값이
		// authentication 에 담겨서 전달된다.
		String username=  authentication.getName();
		String password = (String) authentication.getCredentials();
		
		if (username == null || username.isBlank() || !username.equals("dosm")) {
			/*
		...
		// 생성한 사용자 정보와 권한 리스트를 사용하여 토큰 만들기
	 ...
	 * supports() method 의 return 을 반드시 true 로 만들어줘야 한다.
	 */
	@Override
