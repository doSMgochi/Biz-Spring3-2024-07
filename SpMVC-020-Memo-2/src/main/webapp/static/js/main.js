document.addEventListener("DOMContentLoaded", () => {
  ...
  const onSaveMemo = () => {
    ...
    /**
     * form 에 입력된 데이터를 서버로 보낼 때 보통 submit() 으로 전송한다
    ...
    fetch(`${rootPath}/comps/input`, fetchConfig).then((res) => {
      if (res.status === 200) alert("저장 완료");
      return res.text().then((html) => (listBox.innerHTML = html));
    });
  };
