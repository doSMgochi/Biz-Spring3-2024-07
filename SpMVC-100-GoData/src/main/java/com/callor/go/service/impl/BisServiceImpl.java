...
public class BisServiceImpl implements BisService {
	...
	public List<BisStation> getStations(String station) {
		 ...
		 * 일부 공공DB 서비스중에 text/json type 으로 전달되는 데이터가 있다.
		 * 이러한 데이터(text/json)를 객체타입(application/json) 으로
		 * 변환하기 위하여 중간에 데이터를 가로채서 처리하는 과정 
		 */
		...
		/*
		 * 전통적인 forEach 를 사용하여 list 를 filtering 하기
		 */
...
//		}
	...
	public List<BusArrive> getBusArrive(String busId) {
		...
		log.debug("도착정보 {}",busArriveEntity.getBody());
